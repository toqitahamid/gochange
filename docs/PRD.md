...
- Shadow: `radius: 10, x: 0, y: 4, opacity: 0.05`
...
#### SwiftData Models
- **WorkoutDay**: Scheduled workout templates (Push/Pull/Legs/Fullbody)
- **Exercise**: Exercise library entries within a workout day
- **WorkoutSession**: Completed workout instances
- **ExerciseLog**: Individual exercise performance within a session
- **SetLog**: Individual set data (weight, reps, RIR, completion)
- **RestDay**: Scheduled rest days with reason and notes
- **RecoveryMetrics**: Daily recovery and readiness data (HealthKit + user-reported)
- **RestTimerAttributes**: ActivityKit attributes for rest timer Live Activity
- **WorkoutActivityAttributes**: ActivityKit attributes for workout progress Live Activity
...
- **Widgets**: WidgetKit (App Groups: `group.com.toqitahamid.gochange`)
...
- WatchConnectivity
- Combine
- ActivityKit (Live Activities — rest timer + workout progress)
- WidgetKit (home screen widget — weekly progress)
- FSCalendar v2.8.4+ via Swift Package Manager
